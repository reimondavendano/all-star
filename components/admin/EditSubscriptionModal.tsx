...
export default function EditSubscriptionModal({
    ...
    useEffect(() => {
        if (isOpen && activeTab === 'plan' && formData.plan_id !== subscription.plan_id) {
            loadPlanChangePreview(formData.plan_id, planChangeDate);
        }
    }, [activeTab, formData.plan_id, isOpen, loadPlanChangePreview, planChangeDate, subscription.plan_id]);

    useEffect(() => {
        if (
            isOpen &&
            activeTab === 'plan' &&
            formData.plan_id !== subscription.plan_id &&
            !planPreview &&
            !planPreviewLoading &&
            !planChangeError
        ) {
        ...
        try {
            ...
            if (hasPlanChanged) {
                ...
                if (planResult.warning) {
                    ...
                    setSuccessMessage('Subscription updated. The old-plan prorated invoice was handled now; the new-plan prorated invoice will be generated by the next invoice run.');
                }
            } else {
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div className="p-4 bg-gray-900/40 border border-gray-800 rounded-xl">
                                        ...
                                        <div className="text-xs text-gray-500 mt-1">
                                            {formData.invoice_date === '30th' ? '5th of next month' : '20th of the month'}
                                        </div>
                                    </div>
                                    <div className="p-4 bg-gray-900/40 border border-gray-800 rounded-xl">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-sm text-amber-300/70">
                                                This handles the old-plan prorated invoice now, then lets the next invoice run bill the new-plan remainder.
                                            </div>
                                        </div>
                                    </div>
                                </div>
